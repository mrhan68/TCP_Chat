...
void on_signup_btn_clicked(GtkButton *btn, gpointer data)
{
    ...
    // send to server for checking
    pkg.ctrl_signal = SIGNUP_REQ;
    send(client_socket, &pkg, sizeof(pkg), 0);
    result = signup(client_socket, (char *)username, (char *)password);

    // checking result
    if (result == SIGNUP_SUCC)
    {
        notif_dialog(GTK_WINDOW(login_window), SIGNUP_SUCC_NOTIF);
    }

    if (result == TAKEN_ACC)
    {
        notif_dialog(GTK_WINDOW(login_window), TAKEN_ACC_NOTIF);
    }
}
...
void show_login_window(int *client_socket_pt)
{
    ...
    g_signal_connect(signup_btn, "clicked", G_CALLBACK(on_signup_btn_clicked), client_socket_pt);
